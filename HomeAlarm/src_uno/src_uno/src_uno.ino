...
#define FRONTDOORPIN A5         //read motion sensor on RFID-module
    ...
    void setup() {
        ...
        String username = alarmdata.users.get(0)->name.get();
        ...
        EEPROM_update();    // if alarm is shut down before any action (motion/GSM)
...
// sets processor im sleep mode, wakes up when an interrupt is triggered
    void sleepMode(){
...
// EEPROM functions
    ...
    // saves alarmdata object !!starts at pos. 20 in eeprom array!!
    void EEPROM_update(){
        eeprom_update_block((const void*)&alarmdata, (void*)20, sizeof(alarmdata));
    }
    // nulls the entire eeprom
    void EEPROM_clear(){

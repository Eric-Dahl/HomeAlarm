...
//########### AT-return struktur: (atcmd skickat till modul(inkl.\r(\n))) \r\n (atcmd return) \r\n \r\n (OK\ERROR) \r\n
    ...
    GSMControlUno::SMS::SMS(String raw_data){
	...
	void GSMControlUno::Message::clear(){
		command = "";
		args.clear();
	}

	void GSMControlUno::Message::assign(String newval){
	...
	void GSMControlUno::init() { // set sms text mode(+CMGF=1) and the right char table(+CSCS="GSM")...
		//Serial.begin(9600);
		//Serial.println(F("GSM ctor start"));
		Serial.begin(9600); //115200 not available for autobauding
		delay(200);
	...
	bool GSMControlUno::sendSMS(const String &number, const String &message){  //send an SMS via AT+CMGS
		const String ATCMGS = F("AT+CMGS=\"");
		sendATcommand(ATCMGS + number + F("\"\r"));
		//Serial.println(message);

		//Serial.println((int)free_____Ram());
		...
		if (data_recieved.indexOf(F(">")) != -1){
			...
			//Serial.println((int)free_____Ram());
			Serial.print((char)26);//END_OF_SMS);
			recieveATcommand(15000);	//###############returnvalue
	...
	String GSMControlUno::recieveSMS(const bool deletesms){   //recieve and read SMS   +CMTI: "SM",int location    AT+CMGR=location        if delete=true deleteSMS() will be called and the SMS will be read and then deleted 
	...
	bool GSMControlUno::deleteSMS(const String &location){ //delete an SMS via AT command    location-the location in memory
		const String DELETECMD = F("AT+CMGD=");
		return (Transfer_CMD(DELETECMD + location, 5000) == F("OK"));    //send the AT delete command
    }


	String GSMControlUno::Transfer_OK(const String &ATcommand, const uint32_t timeout){ //send an AT command to the GSM module  then, recieve either the "OK" from the GSM module(return true) or the "ERROR" (or anything else)(return false)
	...
	String GSMControlUno::Transfer_CMD(const String &ATcommand, const uint32_t timeout){  //recieve the AT return value (or command) as a string. If any error the returnvalue is that errorcmd
		sendATcommand(ATcommand);
	...
	void GSMControlUno::sendATcommand(const String ATcommand){
		Serial.flush();
		byte len = ATcommand.length();
		for (byte i = 0; i < len; i++){
			Serial.print(ATcommand[i]);
			delay(5);
		}
		Serial.print(F("\r"));    //added a carriage return to indicate for GSM module that the command ends here
	}

	String GSMControlUno::recieveATcommand(const uint32_t timeout){
		String ATreturn;
		uint32_t starttime = millis();
		//Serial.flush();  (flushes away new arrived data?)               //flush the buffer
		do {
			while (Serial.available()){    //now that we got an answer, we wanna read all data, till the stream ends
				ATreturn += (char)Serial.read();    //append the new data to returnstring
			}
		} while (millis() - starttime < timeout && searchEndCommand(ATreturn) == "");

		return ATreturn;
	}
	String GSMControlUno::searchEndCommand(String &searchstr){

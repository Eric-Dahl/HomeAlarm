/*
!!!!!!!!!!!!  Do not use! It makes the program miss the software signal of an incoming SMS
bool isSMS(){ //checks if interrupt was because of an incoming SMS, eg. the RING pin  was low for ~120 ms     NOT 100% sure
    int i = 0;
    while(digitalRead(2) == false){
        i++;
        delay(10);
    }
    if(60 < i || i < 180)    //lite tilltaget för säkerhets skull
       return true;
    return false;   //else
}
*/
void SMSallusers(const String &message){
    ...
    for(int i = 0; i <= alarmdata.users.size(); i++){
        ...
        if((smsuser->hasRestrictedAccess == false || alarmdata.isExtendedAccess() == true) && smsuser->phone != F("")){ //filters out those with restricted access and those without a number
            sendSMS(smsuser->phone, message);
        }    
    }
}
...
//such as switching it on/off or adding/removing users
    ...
    const String BY_SENDER = F(" by sender");// + sender + F(".");
    Serial.println(F("IN cmdH"));
...
//____________________________________________________________________________________________________________________________________________________________MobiltjÃ¤nster
  ...
  //IMPORTANT!!! - Scan the new RFID-card before adding the user - The latest scanned card will be associated with the new user!!!
        ...
        alarmdata.setExtendedAccess(args[0].equals(F("TRUE")) || args[0].equals(F("1")));
